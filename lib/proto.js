...
/**
 * Lock `key`, `fn(err, unlock)`.
 *
 * The method adds a lock on `key` and queus
 * all operations with the same key so hey are run
 * sequentially instead of concurrently.
 *
 * The given `fn` will receive an error if any.
 * when the lock is aquired it will receive `unlock()` callback.
 *
 * @param {String} key
 * @param {Function} fn
 * @api public
